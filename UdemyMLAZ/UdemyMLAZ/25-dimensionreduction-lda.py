...
try:
    dataset = pd.read_csv('..\\DimensionalityReduction_LDA\\Wine.csv')
except:
    dataset = pd.read_csv('UdemyMLAZ\\DimensionalityReduction_LDA\\Wine.csv')
...
dr = LDA(n_components = 2)
# we add the y_train because LDA is supervised so it is relying on the dependant variable
